...
- **删除原 `shuo config` 子命令族**：用户用编辑器（lazyvim 等）直接编辑 `~/.config/shuohua/config.toml`、`~/.config/shuohua/apps/*.toml` 和 `~/.config/shuohua/post/**/*.toml`。校验/打印归 `doctor`。
- **删除原 `shuo history` 子命令族**：TUI 看历史。脚本要 grep 直接读 `~/.local/state/shuohua/history.jsonl`，schema 见 [SCHEMA.md](./SCHEMA.md)。
- **默认 CLI 不包含 VAD 评测命令**。M10 调研期如需离线评估 Silero，可用开发 feature 构建：
  `cargo run --features dev-vad-probe -- vad-probe --wav sample.wav --threshold 0.5`。
  该命令只读 16kHz mono s16le WAV fixture，不启动 daemon、不录音、不调用 ASR、不产生计费。
- **VAD shadow trace 不是 CLI 命令**。需要真实录音评估时，用 `--features dev-vad-trace` 构建 daemon，并在
  `~/.config/shuohua/config.toml` 里设置 `voice.vad_trace = true`；trace 写入 state dir 的 `traces/`。
...
- **StandardOutPath / StandardErrorPath**：launchd 不重定向就丢 `/dev/null`，看不到崩溃 backtrace
