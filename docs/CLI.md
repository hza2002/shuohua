...
shuo stop             # 通过 UDS 请求 daemon 正常退出
...
- **KeepAlive { SuccessfulExit: false }**：daemon 崩了自动重启；`shuo stop` 走 UDS graceful shutdown，让 daemon 自己退出 0，不触发重启。UDS 不可达时 stop 返回错误，避免用 signal kill 触发 KeepAlive 重启。
