...
> **状态**：已经接入主录音流程（`voice/finish.rs::run_multi_session_recording`）。启用条件：`[voice.vad] backend = "silero"` + `asr/<provider>.toml.idle_pause = true`，两个开关都打开时走多 session 路径，否则走单 session。WebRTC VAD 在真实声学环境里误判率高（风扇/空调嗡鸣谐波会过频域检测、RMS 门无法同时满足灵敏度与稳定性），不适合生产；当前选 Silero。
...
当前代码没有显式 `Voice` enum；状态主要由 `finish::run_recording`、`StateStore` 和 overlay model 协同表达。
