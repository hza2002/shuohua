...
持久化/线协议格式见 [SCHEMA.md](./SCHEMA.md)，CLI 见 [CLI.md](./CLI.md)。
...
**Overlay 反馈通道**：不开第二个 NSPanel，两条通道全部复用主 panel：
...
- `cmd+left_cmd` 退化为 `left_cmd`；`left_cmd+right_cmd`（两侧都按）当前拒绝，以避免 `ModMatcher` 类型膨胀；有真实需求时再加 `BothSides` 变体
- `:double` 后缀仅一个，必须在末尾
...
pub enum AsrEvent {
    Partial { text: String, seq: u64 },                                 // 当前 utterance 尾巴，会被后续 Partial 覆盖
    Segment { text: String, started_at: Instant, ended_at: Instant },   // 句末（server VAD 或 is_last 后）
    Final   { text: String },                                           // provider 可选的 session 最终全文
    Error   { err: AsrError },                                          // 不要混进 Result，让 voice 决定降级
    Done,                                                               // session 终结（is_last + 最后一段已发完）
}

/// 用 thiserror 结构化，voice / overlay / doctor 直接 match 错误类型，零字符串解析。
...
> **状态**：`voice/finish.rs::run_recording` 是唯一公开入口，录音运行期由
> `voice/engine.rs::run` 处理。engine 通过 `RecordingMode::{Continuous, VadPause}`
> 区分 PCM 路由；只有
> `[voice.vad] backend = "silero"` 与 `asr/<provider>.toml.idle_pause = true`
> 同时成立时使用 `VadPause`，否则使用 `Continuous`。Continuous 不构造
> Silero、timeline 或 pre-roll 状态，最终 `sessions[]` 最多一条。
...
`RecordingMode` 只表达固定录音策略；运行态由 `engine::run` 中明确的 Active /
Idle 状态、`StateStore` 和 overlay model 协同表达。engine 统一处理 recording
初始化、ASR event、stop drain、provider finalize、错误/取消和 retained audio，
并返回 `EngineOutcome`；finish 再统一处理 post/dispatch、history 和最终 UI 状态。
engine 不调用 `post::run_chain` / `dispatch::dispatch` / history append，但仍读
`post::AppContext`（数据载体）、`post::app_context::frontmost_app()`（stop 时
取一次 frontmost app）和 `SessionParams.post_chain.name`（overlay header
chain summary 字符串）。

**主动 Done 路径**：任一模式下 provider 主动发 `AsrEvent::Done` 都被视为当前
session 已自行结束；engine 不再向同一 session 发 `is_last`，也不会再等第二个
Done。VadPause 在此基础上额外触发 Idle 等待下一段 speech，避免 provider 自发
结束 session 时被当成 `asr_timeout`。
...
3. **Idle 只暂停 ASR**：VadPause 的 Idle 仍持续持有并读取 cpal stream，用于
   VAD 判断和 pre-roll；Idle PCM 不发送给 provider。Continuous 始终 Active。
...
| 名字 | 类型 | 作用 |
|---|---|---|
| `IdentityProcessor` | 内置 | 透传，等于关后处理 |
| `ZhFilter` | 内置 | 中文语音输入文本过滤：标点/空白/segment 边界/少量语气词 |
| `LlmCleanup` | 内置 | 调 OpenAI 兼容 API（Anthropic / OpenAI / 任意兼容端点）|
...
不提供环境变量或配置项控制日志等级。当前不做 `shuo logs` 命令，也不自动清理
...
- daemon ready
- recording started：`recording_id`、provider、app、mode
...
且 `dev.vad_trace = true` 时可以包含 ASR 正文、高频 ASR event 和 VAD 事件。
...
| TRACE | 当前不用；VAD/ASR 高频观测走 `voice/observer` sidecar |
...
| PCM 通道（callback→consumer） | `tokio::sync::mpsc::unbounded` | 已在真实录音路径验证稳定；Go 版 syscall pipe 都稳跑，mpsc 更轻 |
