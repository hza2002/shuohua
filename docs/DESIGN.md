...
> **状态**：已经接入统一录音生命周期 `voice/finish.rs::run_recording`。生命周期通过
> `RecordingMode::{Continuous, VadPause}` 区分 PCM 路由；只有
...
`RecordingMode` 只表达固定录音策略；运行态仍由 `finish::run_recording` 中明确的
Active / Idle 状态、`StateStore` 和 overlay model 协同表达。两种模式复用相同的
recording 初始化、ASR event、stop drain、provider finalize、错误/取消、
retained audio、post/dispatch、history 和 UI 收尾。
