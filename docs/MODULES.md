...
```
src/
├── main.rs                              # tokio multi-thread runtime；F16 toggle 状态机；3 个 reload subscriber
...
├── state/
│   ├── mod.rs                           # StateStore + StateSnapshot + StateEvent broadcast
...
数据流：F16 → CGEventTap → pipe → mpsc → `Tracker` → tokio main loop。第一次按 = spawn `finish::run_recording` 任务：cpal stream → `DoubaoSession.send_pcm` 流式推、`AsrEvent::Segment` 累积、`StateStore` 同步状态、`OverlayHandle` 推 UI 命令。第二次 F16 = oneshot 通知 task 收尾：drain `stop_delay_ms` 尾音 → send `is_last` → 等 Done（5s 超时）→ segments 直接 concat（provider 自带分隔） → filler pipeline → 剪贴板 + Cmd+V → `history.jsonl` 落一行。配置热重载：notify watcher 监听 `~/.config/shuohua/` → 通过 `watch::Sender` 广播给 overlay / i18n / hotkey 三个 subscriber。

## 未实现（按里程碑）

| M | 新增路径 | 主要新依赖 |
|---|---|---|
| **M4** (下一个) | `ipc/{mod,server,protocol}.rs`（UDS server + line-delim JSON 协议，schema 见 [SCHEMA.md §1](SCHEMA.md#1-uds-协议)），`tui/{mod,panes,keybindings}.rs`（订阅 UDS 看实时 partial / pipeline_step + 滚动历史） | ratatui, crossterm, tokio UnixListener |
| **M5** | `cli/{mod,doctor,service,smart}.rs`（`shuo install` / `shuo doctor` / 智能 fallback：UDS 存在 → 当 TUI 客户端，不存在 → fork daemon + 起 TUI） | clap |
