...
- `sessions[].audio_ms == ended_at - started_at`（recording timeline 上的实际发送窗口）
- 单 session 路径（`idle_pause = false` 或 `voice.vad.backend = "off"`）：`sessions.len() <= 1`，`asr.audio_ms <= duration_ms`
- 多 session 路径：`sessions[]` 按 `started_at` 递增；允许相邻 session 因 resume pre-roll overlap 时间重叠；`asr.audio_ms` 允许略大于 `duration_ms`
