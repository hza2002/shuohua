...
- **`asr.audio_ms`**：Σ `sessions[].audio_ms`，是 provider 实际收到的音频总时长，也是按音频时长计费 provider 的计费事实。当前单 session / 无 overlap 时，`duration_ms - audio_ms` 可近似 client VAD 省下的时长；M10 允许 resume pre-roll overlap 后，省时比例应由 trace 或去重后的 idle 区间评估。
...
- 当前单 session 阶段：`asr.audio_ms <= duration_ms`
- M10 多 session 阶段：`asr.audio_ms` 允许因 resume pre-roll overlap 略大于 `duration_ms`
- 当前单 session 阶段：`sessions[i].ended_at <= sessions[i+1].started_at`（实际只有一条）
- M10 多 session 阶段：`sessions[]` 按 `started_at` 递增；允许因 resume pre-roll overlap 出现相邻 session 时间重叠
...
`sessions[]` 目前永远只有一条（`len == 1`），对应整段 recording。M10 多 session 阶段（VAD 切段、多次 provider session）落地后，writer 侧填多条，schema 字段不再变动。详见 [M10 Multi-session ASR](M10.md)。
