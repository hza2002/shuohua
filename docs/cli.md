...
面向用户的命令按管理对象分组：app/binary 生命周期在顶层，后台 daemon
生命周期在 `service` namespace 下。`clap` derive 实现。
...
`shuo completions <shell>` 支持 `zsh`、`bash` 和 `fish`，生成结果写到 stdout。
推荐写入当前 shell 已经会扫描的 completion 目录。Homebrew 环境常见路径如下：
...
# bash
shuo completions bash > "$(brew --prefix)/share/bash-completion/completions/shuo"

# fish
shuo completions fish > "$(brew --prefix)/share/fish/vendor_completions.d/shuo.fish"
```

如果不用 Homebrew，也可以写入自建目录，但 zsh 必须先把该目录加入 `fpath`。
例如 `~/.zsh/completions` 不是 macOS 或 zsh 默认路径；只有在 `.zshrc` 里配置
`fpath=(~/.zsh/completions $fpath)` 后，放在那里的 `_shuo` 才会被 zsh 发现。
...
- **`update` 不重启 daemon**：更新 binary 后只提示运行 `shuo service restart`；
  不自动停止、启动或 signal daemon。
- **`completions` 只输出到 stdout**。CLI 不猜测用户 shell，也不写入
  `/usr/local/share`、`~/.zsh` 或 fish 配置目录；手动安装和 Homebrew formula 负责选择目标路径。
...
`shuo service install` 生成下面这份 plist 到 `~/Library/LaunchAgents/com.hza2002.shuohua.plist`，然后 `launchctl bootstrap gui/$UID` 它。
