...
HEAD: `feat: add linux service dry-run skeleton`
...
当前正在收敛 Phase 10e Linux systemd user dry-run/status skeleton：Linux service backend 只生成
systemd user unit path/body 并在 `shuo service status` 输出 dry-run 信息；不写 unit 文件、不调用
`systemctl --user`、不 enable/start/stop daemon，也不声明 Linux runtime 可用。
...
- Phase 10e 已完成。下一步可继续把 Linux desktop/hotkey/overlay runtime gaps 细化到
