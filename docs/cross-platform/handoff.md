...
HEAD: `fix: authorize gui placeholder commands`

## 当前 phase

Phase 9af: GUI Static Frontend Global Tauri API 已实现并完成自动验证，等待 commit。
用户手动验证发现 GUI 点击 Refresh 仍完全无变化；根因是静态 HTML 使用 `window.__TAURI__.core.invoke`，
但 Tauri v2 未启用 `app.withGlobalTauri`。本阶段只启用 global API 并让 missing API 显示
`tauri-api-missing`；GUI 仍未订阅 daemon event，所以录音中页面不自动变化仍是预期缺口。
...
- Phase 9m/9n/9o/9p/9q/9r/9s/9t/9u/9v/9w/9x/9y/9z/9aa/9ab/9ac/9ad/9ae/9af 只创建最小 `src-tauri/**` skeleton、静态 placeholder、本地 metadata
...
handoff.md。
Phase 9af GUI Static Frontend Global Tauri API 已实现；先查看最新 commit 和验证结果。
