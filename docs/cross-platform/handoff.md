...
HEAD: `feat: add gui backend event bridge skeleton`

## 当前 phase

Phase 9h: GUI Backend Event Bridge Skeleton 已实现并完成自动验证。下一步可以继续
GUI PoC 前置设计，例如 Tauri permissions/capabilities、首屏指标采集、daemon offline/reconnect
事件桥接；不要直接做完整 GUI。
...
- Phase 9g 只提供连接状态/退避骨架，没有实现真实后台 reconnect task、Tauri event bridge、
  daemon offline UI view model 或指标采集。
- Phase 9h 只提供 GUI backend event bridge 的纯封装，没有实现 Tauri event emission、
  frontend view model、后台 reconnect loop 或指标采集。
...
- 若继续 shared client API，可以设计首屏 metrics/connection timing 的纯记录模型，不新增 daemon
  protocol。
...
handoff.md。
Phase 9h GUI Backend Event Bridge Skeleton 已实现；先查看最新 commit 和验证结果。
下一步在 Tauri permissions/metrics/reconnect PoC 设计、首屏 metrics 纯模型、
