...
HEAD: `feat: add gui first-screen offline shape`

## 当前 phase

Phase 9x: GUI First-Screen Offline Error Display Shape 已实现并完成自动验证。下一步可以继续做
...
- Phase 9m/9n/9o/9p/9q/9r/9s/9t/9u/9v/9w/9x 只创建最小 `src-tauri/**` skeleton、静态 placeholder、本地 metadata
  ...
  first-screen offline/error display shape；
...
handoff.md。
Phase 9x GUI First-Screen Offline Error Display Shape 已实现；先查看最新 commit 和验证结果。
