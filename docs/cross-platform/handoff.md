...
HEAD: `feat: add gui client reconnect state skeleton`

## 当前 phase

Phase 9g: GUI Client Reconnect State Skeleton 已实现并完成自动验证。下一步可以继续
...
- 若继续 shared client API，可以设计 daemon event 到 GUI backend event 的桥接 shape，不新增
  daemon protocol。
...
handoff.md。
Phase 9g GUI Client Reconnect State Skeleton 已实现；先查看最新 commit 和验证结果。
下一步在 Tauri permissions/metrics/reconnect PoC 设计、GUI backend event bridge 设计、
