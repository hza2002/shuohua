...
HEAD: `fix: enable gui tauri global api`

## 当前 phase

Phase 9ag: GUI Manual Refresh Readable Summary 已实现并完成自动验证，等待 commit。
本阶段只让显式 Refresh 后的结果更可读：connected、state、history count、latest preview、timing 和
error summary。GUI 仍未订阅 daemon event，所以录音中页面不自动变化仍是预期缺口。
...
- Phase 9m/9n/9o/9p/9q/9r/9s/9t/9u/9v/9w/9x/9y/9z/9aa/9ab/9ac/9ad/9ae/9af/9ag 只创建最小 `src-tauri/**` skeleton、静态 placeholder、本地 metadata
  ...
  frontend global Tauri API 和手动 Refresh 可读摘要；
...
handoff.md。
Phase 9ag GUI Manual Refresh Readable Summary 已实现；先查看最新 commit 和验证结果。
