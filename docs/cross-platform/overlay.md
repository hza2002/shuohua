...
- 第一版 Windows backend 可以先返回 structured unsupported/degraded 或只提供可编译 skeleton；
  不引入 WebView，不改变 macOS AppKit renderer。
...
- 不引入 `windows` crate，不创建窗口，不进入 daemon 热路径，不依赖 WebView。
...
- `src/overlay/windows/composition.rs` records the future composition backend contract: Win32 HWND host,
  ...
  `IDCompositionVisual3`. The DirectComposition device is created from a D3D11 BGRA DXGI device, with WARP fallback
  if hardware D3D11 is unavailable; this follows the documented `CreateSurface` requirement and avoids the earlier
  NULL-device route that failed with `0x8000000E`. This uses lazy surface creation: startup creates only the device/target/visual tree, then the
  first scene update creates and binds panel/shadow/icon `IDCompositionSurface` objects with real pixel sizes. This
  avoids `IDCompositionDevice::CreateSurface` being called too early during daemon startup. The probe uses the same shadow outset geometry as the Direct2D fallback: the composition
...
- 不引入 layer-shell、GTK/KDE/Wayland crate，不创建窗口，不依赖 WebView。
...
- 不接入 TUI，不实现 Windows/Linux renderer。
