...
- Phase 9ak GUI Event Stream State Forwarding Fix：已完成自动验证，修复 9aj 手动验证发现的
  “不点 Refresh 时 GUI 不变化”：backend stream 现在把既有 `StateChanged` 映射为 frontend 已消费的
  `daemonStatus` payload；不新增 IPC event、不 bump `PROTO_VERSION`、不新增 recording controls。
- Phase 9al GUI Event Stream First-Screen Data Projection：已完成自动验证，backend stream 现在把
  既有 `StatsChanged`、`Partial`、`Segment`、`HistoryAppended` 映射到同一 Tauri event payload，
  frontend 自动更新现有 placeholder 的 live stats/text/latest record 字段；仍不轮询、不新增 IPC
  event、不新增 recording controls。
- GUI PoC 冻结决策：Phase 9al 后暂停 GUI 产品化实现。当前 GUI 只保留为未来复杂 GUI 的
  Tauri/client API/event bridge 证明，不把 placeholder 继续打磨成半成品。下一主线转回
  Windows/Linux 原生 overlay 和非 macOS 可用性。
...
7. 保留 Tauri GUI client PoC 接口；完整 GUI 产品设计后置。
8. 优先完成 Windows/Linux 原生 overlay 和非 macOS TUI/overlay 可用性。
