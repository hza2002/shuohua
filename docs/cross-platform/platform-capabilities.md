...
- `current_platform_capabilities()` 返回当前平台的低风险静态快照；不做权限弹窗、不启动
  daemon、不打开按需 client、不 probe 高风险系统 API。
...
- 用户在 Windows 本机分别验证 compact `.m4a` 和 lossless `.flac` 录音可生成；本 session 复核对应
  history shard 中记录为 `submitted` 且 provider 为 `doubao`。
- 该结论覆盖真实默认麦克风、ASR 提交、history 关联和 native retained-audio conversion，不覆盖多设备矩阵、
...
- `process.probe`：`partial`，backend `open_process_probe`，reason `child_process_exit_runtime_smoke`。
  `OpenProcess` / `GetExitCodeProcess` has tracked a same-user child process from running to exited, but daemon
  crash, abandoned mutex, PID reuse, and permission edge cases still need Windows validation.
...
Phase 10bw 后，Windows `desktop.clipboard` reason 收窄为 `dispatch_clipboard_runtime_smoke`：

- `voice::dispatch::dispatch(text, false)` 已通过 ignored Windows runtime smoke 写入 Unicode 文本，并用 Win32
  `GetClipboardData(CF_UNICODETEXT)` 读回同一文本。
- 这覆盖 voice dispatch -> platform desktop facade -> Win32 clipboard backend 的真实路径，比单独 backend
  write smoke 更接近录音完成后的必需 dispatch 行为。
- 该 smoke 不触发 `SendInput`，不验证目标 App 是否接收 paste，不覆盖 elevation/UAC 和更多 App 矩阵；
  `desktop.clipboard` 仍保持 `partial`。

Phase 10cb/10cc 后，用户完成常见 Windows 目标的真实 full record -> paste smoke：

- `desktop.hotkey`、`desktop.hotkey_suppression`、`desktop.clipboard`、`desktop.text_injection` 和
  `audio.capture` reason 收窄为 `common_targets_record_paste_smoke`。
- 该 smoke 覆盖真实触发键、麦克风录音、VAD/ASR、Win32 clipboard、`SendInput` Ctrl+V，以及两个常见
  foreground targets：Notepad、VS Code text editor、browser text field、Windows Terminal / PowerShell，以及
  中文 IME/composition 场景。
- 仍不能升级为 `available`：remote desktop、UAC/elevation、Office/Teams-style apps、长时间录音稳定性和更多
  设备/权限状态尚未覆盖。
...
Phase 10bv 后，Windows `audio.capture` reason 进一步收窄为 `full_recording_history_smoke`，因为真实
hotkey-triggered recording 已产生 `submitted` history 和 retained audio。它仍保持 `partial`，直到覆盖更多
设备/权限/remote desktop/长时间稳定性矩阵。

## 设计约束

- capability probe 不执行高风险动作。
- probe 不应阻塞 AppKit/window callback。
- permission 诊断应平台化。
...
- 该函数不创建窗口、不读取业务配置、不执行权限或 compositor probe；它只是给后续
  doctor/TUI 接入 renderer 降级信息预留稳定形状。
...
- TUI 消费方式留到单独阶段设计，不让 daemon 热路径或业务层直接依赖 renderer snapshot。
