...
- Client connect uses a raw `CreateFileW`/overlapped client path and then hands the handle to Tokio
  `NamedPipeClient::from_raw_handle`; it does not use Tokio `ClientOptions` generic
  `GENERIC_READ`/`GENERIC_WRITE` access.
- Current desired access is `FILE_READ_DATA | FILE_WRITE_DATA`, enough for JSON-line command/event exchange
...
3. `shuo service start` / `restart`: explicit current-user-session spawn of the current executable with
   `--daemon`, followed by a bounded `DaemonStatus` wait. This is developer/runtime lifecycle control, not
   installation or auto-start registration.
4. `shuo service install --dry-run` or equivalent preview: show intended user logon task/startup registration.
5. Runtime-tested Task Scheduler logon trigger for current user.
6. Optional Startup Apps registration if it provides a better user experience.
...
- `lossless` uses the pure Rust `flacenc` encoder to convert the recorder WAV to FLAC. This path also does not
  require `ffmpeg.exe` or any user-installed runtime.
...
- Capability remains `partial` until full hotkey-triggered Windows recordings for both modes are opened/played back
  on the target machine.

This satisfies the current single-binary dependency policy for retained-audio conversion, but it does not by itself
prove end-to-end recording, history association, Explorer open/reveal, or playback behavior.
...
- `audio.capture` remains `partial/cpal_wasapi`, but its reason is now `common_targets_record_paste_smoke` after
  Windows real microphone recording completed full record -> paste across Notepad, VS Code, browser text field,
  Windows Terminal / PowerShell, and Chinese IME scenarios.
...
- Capability is `partial/wh_keyboard_ll/common_targets_record_paste_smoke` for both `desktop.hotkey` and
  `desktop.hotkey_suppression` after the user verified full record -> paste across Notepad, VS Code, browser text
  field, Windows Terminal / PowerShell, and Chinese IME scenarios. Remote desktop, UAC/elevation, and
  Office/Teams-style apps are still unvalidated.
...
- Capability is `partial/win32_clipboard_unicode/common_targets_record_paste_smoke` after `voice::dispatch`
  writes Unicode text through the real Win32 clipboard path and the user verified full record -> paste across
  common Windows targets. It remains partial until broader target-app and elevation boundary validation is complete.
...
- Capability is `partial/sendinput_ctrl_v/common_targets_record_paste_smoke` after the user verified full record ->
  paste across Notepad, VS Code, browser text field, Windows Terminal / PowerShell, and Chinese IME scenarios.
  Remote desktop, UAC/elevation, and Office/Teams-style apps are still unvalidated.
