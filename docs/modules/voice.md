...
- **PCM 投递必须有界**：每次 `send_pcm` 最多等待内部 2s；超时是 terminal error，包含 stop 阶段的 residual PCM。此时不再 finalize 半截 session，也不进入 post/clipboard/paste。provider 内部还要独立约束命令队列和 transport 写入，避免 voice future 被取消后留下后台任务。
