...
- **history direct response 与 broadcasts**：同一 UDS 连接上 direct command response 按该连接命令顺序返回；`history_changed` / legacy `history_appended` 是 broadcast，和 direct response 的相对顺序不指定，client 必须 coalesce refresh。
...
- `abs(sessions[].audio_ms - (ended_at - started_at).whole_milliseconds()) <= 1`。
...
读侧通过 metadata fingerprint 做稳定读取：读前 fingerprint、scan/read、读后 fingerprint，一致才接受；不一致 retry once。正常请求路径不做 content hash。文件中间损坏、完整坏行、非单调 records、symlink/non-regular shard 会让 stats/analytics 进入 stale/unavailable 或让 page 请求返回 error。
