...
fn windows_overlay_composition_infrastructure_is_fallback_gated() {
    ...
    for token in [
        ...
        "DirectComposition visible overlay is disabled on the current layered-window host",
        "disabling probe and keeping fallback renderer",
        "probe_composition",
    ...
    for token in [
        ...
        "D3D11 BGRA DXGI device",
        "static animation binding",
        "looping state-driven opacity animation",
        "disabled on the current `WS_EX_LAYERED` host",
        "visible output therefore stays on the Direct2D",
        "DirectWrite text",
